...
In this work, we propose to estimate humans' \textit{predictable} run-time initiatives in offline planning. To do that we formalize a simple yet elegant version of Theory of Mind and integrate it with the process of the existing HATP/EHDA framework~\cite{buisan:hal-03684211} (extends  HATP~\cite{CirilloKS09,lallement2018hatp}). 
One main reason to select HATP/EHDA as the basis for realizing our version of ToM is that, it uses factored task model(s) and specifications that make the planning problem more compact and amenable to reasoning, which is less likely with frameworks~\cite{unhelkar2019learning} employing flat models~\cite{levine2014concurrent,unhelkar2019learning}.
Moreover, with this integration, we claim that our contribution offers the robot more clever and proactive alternatives to coordinate and interact with humans.
...
These situations are detected and avoided through different means such as (1) minimal communication with the human to align their beliefs, or 
(2) delaying a robot's action on purpose ensuring that humans could ``perceive'' the key updates rather than being communicated.
The latter shows a proactive robot's behavior but it is currently limited, so we just provide a pilot study and intend to investigate in the future. 
However, we thoroughly study the former case showing how our planning scheme captures a pertinent evolution of individual agents' beliefs and decides if and when belief alignment is needed.
...
The paper\footnote{\textbf{\textit{Attached}} -- benchmarks (details), code + a document with detailed background, related work, formal proofs, and more results.} is structured as follows: 
...
After that, Section~\ref{sec:formal-props} provides changes in the existing problem encoding and relevant formal properties of the new planning scheme. 
Followed by Section~\ref{sec:eval} discussing empirical evaluation, and showing both qualitative and quantitative results. 
We then conclude the paper with a discussion.  
...
The section touches on all relevant aspects: I.e., Theory of Mind, particularly, ToM employed in HRI, state-of-the-art planning approaches and models used in similar collaborative settings, and agents' communication that is critical for collaborative planning. 
And, since humans' beliefs are modeled and exploited in our work, we also discuss relevant epistemic planning frameworks. 
We explain where exactly our main idea lies at their intersections.   

\paragraph{Theory of Mind in HRI}
...
It supports the robot's capabilities to do spatial reasoning w.r.t. the humans and tracks their activities. 
In particular, it manages the execution of shared plans in an object manipulation context and shows how a robot can adapt to human decisions and actions and communicates if needed.
...
But these frameworks assume that a joint task is established before planning. Moreover, generally, they produce explicitly coordinated, shared HR plans that are legible and acceptable by humans --- they are assumed to be controllable in some sense, such that the techniques rely more on the replanning aspect.
...
Literature propose to investigate how to create a reasonable model of humans and how to obtain task knowledge, e.g.,~\cite{unhelkar2019learning}. 
HR task planning is also modeled as an optimization problem and solved using multiple cutting-edge research, e.g., mixed integer programming used in the outstanding work done by Vats, Kroemer, and Likhachev (2022).
% ~\cite{VatsKL22}. 
Task knowledge can be gathered offline from human psychologists and expert engineers~\cite{levine2014concurrent,CirilloKS09}; 
% comm - wang2018robot
or can be learned via human tutors or from demonstrations~\cite{koppula2016anticipatory}; or a Markov model for sequential decision-making can also be learned from a partial specification of human behaviors~\cite{unhelkar2019learning}. 
% 
...
We choose a task specification based on HTNs for representing the HRC problem compactly. The most appropriate framework to be extended based on that is HATP/EHDA.
...
is used to align an agent's belief, clarify its decision or action, fix errors, etc.~\cite{tellex2014asking}.
% comm - sebastiani2017dealing
...
Recent work deals with an explicit usage of communication actions in planning~\cite{nikolaidis2018planning,roncone2017transparent,sanelli2017short,UnhelkarLS20}. 
% - comm - BuisanSA20
...
% For basic terminologies, notations, and definitions related to hierarchical task networks, problem specifications, and solution, refer to~\cite{naubooks0014222}. 
For basic terminologies, notations, and definitions for hierarchical task networks,
% , problem specifications, and solution, refer to
see~\cite{naubooks0014222}.
% \begin{definition}
...
We briefly describe the HATP/EHDA framework and discuss its ability to capture a broad class of HR collaborative planning scenarios~\cite{buisan:hal-03684211}. 
...
It can also ask the human for occasional help to accomplish its task or manage the creation of shared tasks, handle human's reactions modeled explicitly via triggers, etc. Ingrand \textit{et al.}'s PRS based system to supervise and control reports triggers.  
% ~\cite{ingrand1996prs
...
It uses specific actions to synchronize agents' plans. {\em IDLE} is inserted into the agent's plan when its task network is empty, and {\em WAIT} when it does not have regular applicable actions. First, the framework builds the whole search space by considering all possible, feasible decompositions.
% ~\cite{buisan:hal-03684211}. 
Then, it can adapt off-the-shelf graph search algorithms, e.g., the well-known algorithms like $A^*$ and $AO^*$, and consider social cost, plan legibility, and acceptability~\cite{alili2009task}, etc., to search for the agents' \textit{joint solution plan} defined next, extends the definition of the HTN's solution for the HATP/EHDA case. 
Note that an implicitly coordinated plan for the robot is contained within such joint solution plans. 

\begin{definition} \label{def:joint-sol-plan}
(\textbf{Joint Solution Plan}.) 
{The solution for $\mathcal{P}_{hr}$, is represented as a tree or a graph, i.e., $G=(V,E)$. Each vertex ($v \in V$) represents the robot's belief state, starting from the initial belief. Each edge ($e \in E$) represents a primitive task that is either a robot's action $o^{r}$, or a human's action $o^{h}$. $G$ gets branched on the possible choices ($o^{h}_1$, $o^{h}_2$, ..., $o^{h}_m$). 
}  
\end{definition}
...
A simplifying assumption the planning scheme makes is that if the actions of the human and robot at a given time stamp are conflicting, then they need to be modeled and handled explicitly, helping the parallelization to be sound~\cite{ShekharB20}.
On the other hand, this assumption of a single-agent acting at a time comes up with an advantage. As humans are uncontrollable one could think of possible joint actions (the humans' next predicted action could also be \textit{delay} or \textit{leave} among others), this assumption often enables the scheme to explore a broader search space. 
...
The formalization below is based on the standard state-variable representation~\cite{naubooks0014222}, while we abuse the standard notations slightly, sometimes, to maintain the flow of the discussion.
...
\textbf{(State Variable Function.)} It is $f_{svs}:(?g_1 (gr_1), ?g_2 (gr_2), ..., ?g_k (gr_k),\mathcal{S})\rightarrow ?g_{k+1} (gr_{k+1})$. 
\end{definition}
Here, $svs$ is \textit{state-variable symbol} (\textit{attribute} name), while each \textit{term} with ``$?$'', can be instantiated with a \textit{constant} from its respective \textit{group} (inside `$()$') appears with the term. 
E.g., to model agents' location, we use $f_{\textit{AgtAt}}:(?a (Agents), \mathcal{S}) \rightarrow ?r (Rooms))$, representing for each agent and each possible legal state ($s_i \in \mathcal{S}$), we know where the agent is located in $s_i$. 
...
The agents' SA processes are modeled at the symbolic level and utilized in planning for better anticipation of what can happen and what the agents can know following their estimated sequence of actions during execution. 
...
We note that SA processes are currently modeled as \textit{deterministic} processes.
In line with DA's work, such evaluation is generally done precisely with \textit{geometric reasoners} at run-time, based on the robot's perception and supervision components, to adapt to ``unpredicted'' human initiatives. 
However, without the availability of run-time data from perception, or a learned model obtained otherwise, it would be less intuitive to consider such reasoners in offline planning. 
...
% \end{definition}
\begin{definition} \label{def:pssav}
    \textbf{(Place Specific State Attribute Function.)} A {\em grounded} state-variable function {\em aka} a state attribute of a given state is explicitly associated with a place. In general, 
...
We provide formal definitions for the intended SA processes based on the \textit{place-based observability} description. 
...
Hence, through this process, an agent can learn about the \textit{observable} effects of an action performed earlier by others. 
...
The \textit{Inference} process models agents' ability to reason about, and deduce non-observable facts in their surroundings under certain conditions. Thus, non-observed actions that include inferable effects create a belief divergence and cannot be fixed like in the former case.
...
Agents' plans (always) start with a dummy action with no precondition and effect such that the \textit{Observation} process is executed and updates initial observable facts.


Note that we do not  consider cases where the robot's beliefs can diverge, too, assuming that, while not being observed, humans only make deterministic moves. Hence, regardless of being co-present, the robot always updates its belief with both the \textit{inferable} and \textit{observable} effects.
...
The first is through minimal communication, which is a formally proven, robust and safe method. The second is through delayed robot actions, which is more conceptual as we have only a pilot study available. But it illustrates the proactive behavior it can provide to the robot.

Note that even though we do not explicitly model uncertainties in what humans believe, we deal with them nonetheless. 
Indeed, when such uncertainties arise --- especially when the robot acts while the human is not co-present --- we assume that humans would think that nothing has changed until they really ``learn" a change. 
...
This affects the scheduling of \textit{when} the robot communicates with the human.
% (discussed later). 

% Currently, as soon as a belief divergence affects the next human actions, it is considered as \textit{relevant} and needs to be tackled. 
% Although we keep the notion of relevancy quite simple in this context, 
However, it could be interesting future work to check in a principled way, the relevance of a belief divergence 
...
this attribute-value pair to $\varphi_j$ \textit{via} the action $ca_{\varphi_i, \varphi_j}(f_{\textit{svs}}(...),q)$, if the following conditions (i.e., $ca$'s precondition) meet: 
(1) For $\varphi_i$, 
...
% Currently, $\mathit{CA}$ is inserted in the plan just before the next human actions that would differ without belief alignment. But it could be interesting to find in the current plan trace, the best place to insert the communication actions.
% $/{ca/}$.  
...
Nevertheless, we see it worth mentioning and discussing it briefly as it illustrates well the possible proactive behaviors the robot can be endowed with --- thanks to our contributions. 
More details and preliminary results are provided in Appendix D.
...
Our solver shares a similar high-level procedure as the existing solver. 
% but with a better estimation of the evolution of agents' beliefs.  
...
In the first domain, we highlight subtleties the existing solver overlooks and how our approach is aware of them, updates beliefs, and effectively manages divergences. 
% We discuss the 
Plans obtained in two scenarios are shown in Fig.~\ref{fig:scenarios}. Assume that the agents are in \textit{Kitchen} and the human begins. Scenario~(A): Captures the agents' plans when the pasta is in \textit{Room}. 
Scenario~(B): 
...
\begin{itemize}
    ...
    Moreover, \textit{Inference} later updates about \textit{SaltInPot} since the human will be co-present.
\end{itemize} 
...
Three boxes have been considered in the Box domain.
...
We believe our modeling-level SA proposals could fit in any other planning approach framing multi-party systems having one controllable agent while can only hypothesize remaining agents' behaviors  (e.g., human-centered AI). Thus, we put an effort to give our contribution a ``generic" description.
...
Agents' SA models only assess new facts and cannot refute a false belief. 
% Currently, our formalism cannot refute something believed by an agent through SA, it can only assess new truths. 
...
Hence, such issues do not affect the completeness. 
Moreover, if necessary, our planning approach \textit{handles} such cases as relevant divergences and communicates to the human.
