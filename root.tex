...
For a seamless human-robot collaboration, it is essential to integrate a task planning framework~\cite{lallement2018hatp} or a scheduling framework~\cite{ferreira2021scheduling} in an \text{online} joint behaviour execution scheme. 
...
We implement this novel concept for HR collaborative planning, prove its soundness and completeness, discuss its effectiveness qualitatively, and show experimental results on three novel domains. Benchmarks (details), code, and a document detailing the background, related work, and more results are provided. \textbf{??VERIFY THIS PARA??}
...
Our notion of an ``observable fact,'' classified into: observable from action and observable from the state, can roughly be associated with the restricted epistemic logic presented and applied in planning applications~\cite{CooperHMMPR21}. 
Also a part of our high-level idea is in line with the concept of perspective shifts epistemic planning based on extended DEL~\cite{engesser2017cooperative}. 
DEL-based representation are rich and can model scenarios involving nested
perspective-taking as  compared to our first-order representation used for maintaining agents' distinct beliefs. 
In~\cite{KR2021-12}, this notion of prospective shifts is extended to provide a foundation for producing implicitly coordinated human-robot plans such that they do not have to negotiate about and commit to a joint policy at plan time. In our understanding, the latter produces HR policies that are not socially awkward which is essentially the aim of such research, but they do not model humans as uncontrollable.  
...
We introduce SA models to be able to evaluate the observability of the agents during the planning process. Such evaluation could be done more precisely with some geometric reasoning, but it would be very challenging to use at planning time. This type of reasoning will be done at execution time using perception and supervision to handle unplanned human initiatives. 
...
\end{definition}
Here, $\mathit{loc}$ is a location specific symbol for {\em place specific attribute}, while the function captures and maintains the place associated to a state attribute $f_{svs_i}$ of a given state.
...
Such mappings can change when an action modifies the \textit{place} of observability of an attribute, e.g., 
...
to either $\observable$ or $\inferable$, i.e., 
$f_{\textit{obs}}: (f_{svs_i},\mathcal{S}) \rightarrow  \{\inferable, \observable\}$.
\end{definition}
...
On the other hand, when a state-variable function ($f_{svs_j}$) belongs to $\inferable$, it means that its value \textit{cannot} directly be observed. However, an agent can, in the right conditions, predict or infer its \textit{exact} value. 
...
First, the relevant divergence is solved by the communication solution to create a safe branch in the solution plan tree. After, we check if the relevant divergence was already present in the initial human belief or if it is due to a non-observed action, we proceed only in the latter case. Next, we identify the diverging action by sequentially applying the already planned actions while monitoring when the relevant inferable attribute diverges. Once identified, a new branch is created before the diverging action and "\textit{Delay}" actions are inserted in the robot's plan until the human is \textit{co-present}. Then, the diverging action is inserted and the nominal planning process is resumed.
...
To apply this delaying solution the relevant divergence must be due to only one \textit{inferable} attribute and the agents must co-present at least once after the diverging action. Thus, delaying the robot action to fix the divergence isn't always feasible. Therefore, this solution is used along with the communication solution, which is always working, and we later select the best one for the robot. However, this selection is based on accurate \textit{plan evaluation} which is currently lacking in this scheme and out of the scope of this paper.
...
Focus on the following two attributes associated to \textit{Kitchen}: For a given state, $s_i \in \mathcal{S}$, $f_{\textit{SaltInPot}}^{s_i} \in \{\textit{true, false}\}$ and $f_{\textit{stove}}^{s_i} \in \{\textit{on, off}\}$ such that only $f_{\textit{stove}}^{s_i}$ is \textit{inferable}, all others belong to $\observable$.
