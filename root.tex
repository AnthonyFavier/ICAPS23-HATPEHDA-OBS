...
% NEW
% While several works from the literature consider Theory Of Mind at execution time to adapt the robot's behavior online, we show how taking such considerations already at planning level can benefit human-robot collaboration.
% Building on earlier work about Human-Aware Task Planning, we describe a new conceptual approach that:
...
% More specifically, we explain where and how Situation Assessment (SA) is inserted in the planning process. Thanks to this, we show how ambiguous situations due to estimated false human beliefs can be detected and avoided by different means (communication or delay robot actions).  
...
% A recent scheme (HATP/EHDA) explicitly models humans as non-controllage agents and aims to only plan for the robot's behavior which is implicitly coordinated with the estimated non-deterministic human's behavior.
% In this paper, we propose a contribution which will allow a better offline estimation and exploitation of the human's mental state. 
% Some Situation Assessment (SA) processes are modeled and inserted in the planning process, distinctly for each agent.  
% These models are based on the \textit{co-presence} and two types of action effects, (1) facts that are inferred while performing an action or observing an action being executed, and (2) facts that can be observed by an agent at anytime. Thanks to this addition, we show how ambiguous situations due to estimated false human beliefs can be detected and avoided by different means (communication or delay robot actions).
...
% anthony-v4
It is essential for collaborative robots to consider Theory of Mind (ToM) when interacting with humans.
Several works integrate ToM when executing collaborative plans to adapt online the robot's behavior to unplanned human initiatives. 
However, having such considerations during offline planning may offer smarter and more proactive alternatives to the robot.
Recent offline planning schemes like HATP/EHDA explicitly model humans as \textit{uncontrollable} agents and aim to plan for robot's actions while \textit{implicitly} coordinating them with the estimated non-deterministic human's behavior. These schemes are suitable for and would benefit from the integration of some ToM concepts to better anticipate the human's behavior.
Hence, based on such existing scheme, we propose a contribution that will allow a better offline estimation and exploitation of the human's mental state. 
Some Situation Assessment processes are modeled and inserted in the planning process, distinctly for each agent.  
These models are based on \textit{co-presence} and two types of action effects: (1) facts that are inferred while performing an action or observing an action being executed, and (2) facts that can be observed by an agent at any time. Thanks to this addition, we show how ambiguous situations due to estimated false human beliefs can be detected and avoided by different means (communication or delay/postpone robot actions).
We implement our new conceptual approach, prove its soundness and completeness, discuss its effectiveness qualitatively, and show experimental results on three novel domains.
...
% shashank-v2
% Considering Theory of Mind (ToM) when executing a shared plan enables a robot to adapt its behavior online. Recent work on online execution and replanning tracks the progress of the overall task from both humans' and robots' perspectives. 
% Every time the human part of the new plan is communicated along with other essential task details, and the execution continues. 
% Here, ToM is used to handle uncertainties caused mainly due to humans' \textit{unplanned} initiatives at run time, e.g., their temporary absence or distractions, which prohibit humans from entailing knowledge about the exact execution status. 
% Online approaches are dependent on plan sharing, which can be costly, and committing every time to a new plan can be restrictive. To the contrary, recent offline solvers provide more latitude to humans by discarding the idea of shared planning while planning only for the robot's behavior. In addition, they consider humans \textit{uncontrollable} and hence only anticipate their  non-deterministic behaviors and generate \textit{implicitly} coordinated joint plans for a (non-)shared goal. 
% % In this paper, 
% Building on existing work, we describe a new offline planning approach that formalizes ToM to maintain agents' individual mental models, which helps anticipate humans' future initiatives (their spatial perspective via \textit{coexistence}). 
% % for managing their mental models. 
% In particular, we formalize a version of situation assessment (SA) with perspective-taking and use it in our new planning approach. We propose precisely where and how SA is inserted in the planning process. 
% Our solver produces more robust and better coordinated plans while estimating the human mental state, encouraging the robot to communicate about humans' \textit{relevant} false beliefs, but only minimally and in a principled way. 
...
\end{definition}
% A \textit{domain} is a 3-tuple $D=(P, O, M)$ where $P$: set of primitive propositions, $O$: set of tasks, and $M$: set of methods. 
% A task $o \in O$ is a primitive task described as $o=(name(o), pre(o), \textit{eff}(o))$, i.e., its name, precondition, and effect, respectively. 
...
% We define a joint solution plan as follows (extending Definition~\ref{def:htn-sol-plan} for the HATP/EHDA case).
...
% The requirement from this solution tree is that 
...
% 
% This work abstractly employs the main ideas of situation assessment based on spatial reasoning and observability during planning~\cite{devin2016implemented}. 
% The goal is to predict the situation assessment of a collaborative human agent to have a better estimate of their knowledge and next possible actions, and thus, produce better plans.
...
% An \textit{inferrable} state attribute is an attribute that can't be observed but whose value can be inferred in certain condition. (salt example to illustrate ? mention observing action execution ? cartoon ?) %BELOW
...
% Based on these formalisms, the dual-HTNs specifications are \textit{transformed} and made more specific by the modeler. Our solver uses the transformed dual-HTNs for the deliberation process. A description of how the applicability of action is affected, more precisely, how its precondition and effect are handled during planning, appears in the following sections. 
...
% To make it unambiguous, say the \textit{first} action executed is a \textit{dummy} with no precondition and effect.
...
% (and one may use speech modality), 
...
% , and to be used in only special circumstances during planning.
% , 
% and they are used only in special circumstances during planning.
...
% First, we note that we \textit{put} an effort to make our main contributions to have a ``generic'' description. Hence, they are not limited to only our intended HATP/EHDA framework, which we plan to enhance in this paper.
% in this work.

% The solver is based on the HATP/EHDA framework that is the underlying planner~\cite{buisan:hal-03684211}. 
% However, 
% Only the required changes in the \textit{old solver} are discussed for planning with communication. 
%
% Suppose an agent applies an action: First, the belief states of all agents co-present with this agent get updated with the action's inferrable effect. Later, the \textit{situation assessment} process is used for every other agent to assess the changes (captured via observable state attributes). 
% As a result, their beliefs are updated by the observable effects if the required conditions are satisfied. We already discussed these subroutines in detail in earlier sections.
% 
% 
% 
...
% in planning. 
% (Recall the sender-receiver pair discussed earlier.) 
% The approach identifies whether the receiver's belief divergence is \textit{relevant}, and thus if the receiver's belief needs to be corrected.
% At this stage, suppose belief \textit{updates workflow} is already called, and the sender has reasoned out and decided to tackle the receiver's divergent beliefs.
...
% unaligned $\mathit{B}_{\varphi_r}$ and $\mathit{B}_{\varphi_r}$.
% 
...
% If so, 
% Now, each attribute for which the receiver's knowledge differs w.r.t. the ground truth (or $\mathit{B}_{\varphi_r}$) is identified.
% 
% And, that at least one divergence (or one attribute-value pair) has an adverse effect. 
% Therefore, only those essential ones need to be identified and aligned. 
% Then, it decides which of these attributes and their values must be sent by the sender (\textit{i.e.}, the robot), supporting the idea of communicating minimally and avoiding verbosity. 
% Once we determine that, the receiver's (\textit{i.e.}, human's) belief gets updated accordingly, ensuring that the remaining false knowledge of the receiver's updated belief is \textit{not effective}, which also means that the belief divergence is not relevant anymore. 
...
% after each stage of HATP/EHDA's plan elaboration. 


% \subsubsection{Delaying Actions to Avoid Divergences}
% To reduce communication, the framework should allow the robot to, sometimes, \textit{delay} its action(s) with an \textit{inferrable} effect. E.g., if the communication is too costly, the robot can refrain from \textit{adding} salt until the human arrives in the kitchen. 

% When a belief divergence is \textit{relevant}, following the current branch, we trace it back to the stage where the action creating this divergence is applied. We then initiate another branch from this stage and let the robot {\em apply} \textit{DELAY}s until the agents are co-present while the rest continues like previously. If the agents never happen to be co-present, we delete this branch.
% The main idea here is to exploit our observability-based formalism more such that HATP/EHDA allows the robot to delay its actions in such scenarios. Of course, the trade-off between using communication and \textit{DELAY} actions will affect the final solution plan tree. Currently, we explore this aspect only prototypically, and it works only when the divergence is due to just one inferrable attribute.
...
% Ours uses a more elaborated encoding formalized earlier. 
...
% \begin{lemma}
% The size of the new encoding for primitive propositions is worst-case linear in the size of $P$.
% \end{lemma}
% \begin{proof}
% Its proof follows the above argument.
% \end{proof}

% consider them to make a good theoretical contribution 
% 1. Is the new solver based exactly on the old? Note that earlier human was able to act as per his belief and to avoid problematic situations we used triggers.

% 2. We take the original dual-HTN then transform state properties to have observability and location specific details (indicates that our specification in use, is less abstract), and accordingly we transformed the primary actions, too. Worst-case analysis should consider that we picked each attribute and classify it to OBS or INF and associate it to a place.

% 3. Are we able to support everything with the help of this transformation which we were technically supporting with the old specifications?

% 4. It works at two levels: Specification and Algorithmic levels 

% 5. All of them are going to affect the theoretical properties of the new approach.   
...
% , e.g., a primitive decomposition to $w_0$. 
% Moreover, each precondition for an agent's action is either provided by: its own action executed previously, 
% another agent's action occurred in the previous time step (this precondition is \textit{inferred}), 
% by another agent communicated previously, 
% or \textit{assessed} by understanding a situation on or before the current time stamp. 
% Of course, that agent’s action supplying the precondition (an attribute belonging to $\observable$ and its value) is scheduled before and cannot be destroyed. 
...
% since the solution conditions are achieved. 
...
% If the underlying scheme exhaustively generates all possible plan elaboration, 
% a solution for the relaxed problem 
% that the underlying solver can find, which means, this formalism after extending each proposition in $P$, and making it \textit{inferrable} at each \textit{place}, is also solvable. Then the proof is simple. 
% Our solver will always be able to receive a planning stage it will find a solution with the help of situation assessment, actions' effect inference, and communication.    
\end{proof}
\end{theorem}
...
% Only the human can execute the last two actions and can either start from \textit{Front} or \textit{Rear}. 
...
% First, qualitative analysis in the first domain to discuss important subtleties the old solver \textit{overlooks} and how ours is \textit{aware} of them, updates belief, and effectively manages divergences. 
% Then, quantitative studies compare the solvers in all three domains.

% 
\subsubsection{Qualitative Analysis}
% Consider the first domain. We discuss the plans obtained in two different scenarios as shown in Figure~\ref{fig:scenarios}. Assume that both the agents are in \textit{Kitchen} and the pasta is in \textit{Room}. Scenario~(A) captures the agents' plans when the robot starts while Scenario~(B) shows when the human starts. Let us change the scenario: Suppose, in hindsight, the pasta is moved to \textit{kitchen} such that the robot knows it, while the human still has the old belief. For this, Scenario~(C) captures the plans obtained when the human starts. 
...
%         \textit{Car} & 12.5\% & 73.2\% & 26.8\% & 100\% & 83.6\%\\
%         \hline
%         \textbf{Average} & 18.7\% & 77.8\% & 22.2\% & 100\% & 69.1\%\\
%     \end{tabular}
%     \end{adjustbox}
%     \caption
%     {
...
% We designed three new domains to test our solver and compare it with the existing solver and the 
...
%Therefore, if the \textit{IDL} case occurs, it is understood that the task specification is erroneous. 
...
% do not handle initial belief divergences (via modeling triggers explicitly). 
...
% One crucial observation is our solver often uses more communication actions in the car maintenance domain, which is different from the other two. It purposely creates non-relevant belief divergence, e.g., whether the robot stored the gallon in the cabinet is irrelevant to the human.  
% But seeing the old solver's success rate (12.5\%), we conclude that this domain needs more communication than the other two. Indeed, after a close analysis of the plans generated, we found \textit{approx.} 72\% of the plans had divergent human beliefs at least in one branch after the last (leaf) action was executed (i.e., irrelevant divergences never got communicated).  
...
% **In the car maintenance domain, the position of the oil bottle isn't relevant for the human. Thus, when Human isn't observing Robot storing the bottle a non relevant belief divergence is created (71.9\%). This divergence has no influence on Human actions so no belief alignment is performed (implies less communication). 
...
% Compared to the old solver, 
% This new planner produces consistent and more robust plans. 
...
% , which we plan to enhance in this paper.
...
% We informally introduced the option to delay robot's actions. We study this aspect separately and in a prototypical way. We have an initial success as we empirically found situations where it would be helpful and reduce overall communication requirements depending on the cost trade-offs. We intend to bring this idea to our elaboration process.
...
%     \item Consider applicability of COM action (co-presence precond?)
% \end{itemize}
