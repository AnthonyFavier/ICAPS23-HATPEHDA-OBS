...
For a seamless human-robot collaboration, it is essential to integrate a task planning framework~\cite{lallement2018hatp} or a behavior scheduling framework~\cite{ferreira2021scheduling} in an \textit{online} joint activity execution scheme. In~\cite{PupaS21}, planning information is dynamically integrated into the scheduling framework, while Devin and Alami~(\citeyear{devin2016implemented}) (henceforth, DA) propose to consider ToM when interacting with humans to react to their \textit{unplanned} run-time initiatives,
% . 
...
One straightforward approach is to have a mechanism that somehow adapts the main idea of one such (flexible) scheme in the DA's online framework. 
However, in this work, we propose to estimate humans' \textit{predictable} execution-time initiatives in offline planning. And for that, we formalize a simple yet elegant version of Theory of Mind, and we implement it within the planning process of the HATP/EHDA framework~\cite{buisan:hal-03684211} --- which extends HATP~\cite{alami2006toward,lallement2018hatp}. We claim that our contribution can offer a collaborative robot more clever and proactive alternatives to coordinate and interact with humans.
One of the main reasons to select HATP/EHDA as the basis for realizing our version of ToM is that, HATP/EHDA uses factored task model and specifications that make the problem more compact and amenable to   reasoning~\cite{naubooks0014222}, which is less likely with flat models~\cite{levine2014concurrent}.
...
It then decides if and when belief alignment is needed and achieves it by communicating with humans, but only minimally and in a principled way. To add a different flavor, for the latter case, we provide a pilot study in \textit{appendix} and intend to investigate in the future.   
...
Suppose the robot has access to your behavioral model. It assumes that you are cooperative, conforming, and rational concerning the joint task/goal but cannot be administered like an artificial agent --- a fully controllable agent. 
...
We choose hierarchical task networks based task specifications for representing a problem compactly to realize our main idea. And the most appropriate framework to be extended is HATP/EHDA.
However, our modeling-level SA proposal could fit in any other planning approaches framing multi-party systems having one
...
A part of our high-level idea is in line with the concept of perspective shifts in epistemic multi-agent planning~\cite{engesser2017cooperative}. In particular, our notion of an ``observable fact,'' classified into: observable from action and observable from the state, can roughly be associated with the {\em restricted} epistemic logic presented and applied in an planning application~\cite{CooperHMMPR21}. 
...
Each agent is convinced about their beliefs of the ground truth and considered trustworthy when communicating with others~\cite{fabiano2021multi}.
...
We formalize our intended concepts of ToM and use them for planning. Agents' SA processes are modeled at symbolic level and utilized for a better anticipation of what can happen and what the agents can know following their execution traces. To make it more precise and to simplify the exposition, we associate the models with the notion of \textit{Places}. 
...
We introduce SA models to be able to evaluate the observability of the agents during the planning process. Such evaluation could be done more precisely with some geometric reasoning, but it would very challenging to use it at planning time. This type of reasoning will be done at execution time using perception and supervision. 

Our symbolical place-based observability model is abstractly inspired by standard execution time observability conventions relevant for the current context~\cite{devin2016implemented}. 
...
Agents are always ``situated'' in a place or moving between two places. 
\begin{definition} \label{def:copresence}
\textbf{(Co-presence.)} Two agents situated in the same place $p \in \mathit{Places}$, in a given state $s_i \in \mathcal{S}$, are said to be \textit{co-present} in this state $s_i$.
\end{definition}


\begin{definition} \label{def:pssav}
    \textbf{(Place Specific State Attribute Function.)} A {\em grounded} state-variable function, {\em aka} a state attribute of a given state, can be explicitly associated with a place. More generally, 
    $f_{loc}: (f_{svs_i},\mathcal{S})) \rightarrow Places \cup \emptyset / \bot$
\end{definition}
Here, $\mathit{loc}$ is a location specific symbol for {\em place specific attribute}, while the function captures and maintains the place associated to a state attribute $f_{svs_i}$.
...
This explicit mappings suggest that the attribute is \textit{observable} in its dedicated place.
Such mappings can change when an action changes \textit{place} of observability of an attribute, e.g., 
while \textit{holding} a cup the robot \textit{moves} to the adjacent room. 
These updates are currently manually handled, carefully.
...
to either $\observable$ or $\inferrable$, i.e., 
$f_{\textit{obs}}: (f_{svs_i},\mathcal{S}) \rightarrow  \{\inferrable, \observable\}$.
\end{definition}
...
We generalize it further, assuming an attribute is either \textit{observable} or \textit{inferrable}, and hence, relaxing the individual state based restrictions, that means, the above expression can be further simplified to, 
% $f_{\textit{obs}}: (f_{svs_i}(?g_1(gr_1),...,?g_k(gr_k))) \rightarrow \{\inferrable, \observable\}$.
$f_{\textit{obs}}: (f_{svs_i}) \rightarrow \{\inferrable, \observable\}$.
...
When a state-variable function ($f_{svs_i}$) belongs to the class $\observable$, it indicates that, under certain requirements, an agent can \textit{assess} its \textit{exact} value.
On the other hand, when a state-variable function ($f_{svs_j}$) belongs to $\inferrable$, it means that its value \textit{cannot} directly be observed. However, an agent can, in the right conditions, predict or infer its \textit{exact} value. 


\subsection{Modeling Situation Assessment processes}

Based on the Place-Based Observability model we just introduced, we model situation assessment with the following two processes that will be used in the planning process/deliberation process. 

Note that the robot is always aware of the ground reality. Hence, technically, the following processes are only effective for the human agent. 
...
The \textit{Observation} process models the ability of agents to observe their surrounding and update their belief based of the observed facts. Hence, though this process, an agent can learn about the \textit{observable effects} of an action performed earlier. 
...
The \textit{Inference} process models the ability of agents to reason and deduce non-observable facts from their surrounding. Thus, non-observed actions including inferable effects create belief divergences that the agent can't fix with observation. 

\begin{figure}[t!]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/cartoon_obs(2).png}
    \caption{
    While Human is grabbing a spoon in the kitchen, Robot adds sugar to the cup before pushing it forward. \textit{Assessing} the new situation after the two actions, Human can only ``know'' the cup's new position (\textit{observable}), but not \textit{SugarInCup} (\textit{inferrable}). Human would know about the sugar if they observed Robot acting.
    }
    \label{fig:cartoon}
\end{figure}
...
To better understand the proposed place-based observability model and its associated situation assessment processes, let's consider a scenario with a cup of coffee and some sugar on a table as depicted in Figure~\ref{fig:cartoon}.
Initially, the cup is far form the human and without sugar. 
% The human leaves the place (i.e. not \textit{co-present}) and, meanwhile, the robot adds some sugar to the cup before pushing it towards the human side of the table.
The robot estimates that the human will go in the kitchen to grab a spoon, and thus not be co-present. Meanwhile, the robot adds some sugar to the cup before pushing it towards the human side of the table.
Once the human is back, they can notice that the cup is close since it is an \textit{observable} fact (\textit{Observation} process). But the human has no way to tell if there is sugar in the cup since it's an \textit{inferrable} fact. However, without leaving the place, the human would have observed the robot adding sugar and inferred the presence of sugar in the cup (\textit{Inference} process). 
...
\begin{enumerate}
    ...
    \item \textbf{Inference}: Co-present agents update their belief with the inferrable effects.

    \item \textbf{Observation}: All agents update their belief with their respective observable facts.
\end{enumerate}
...
*Note that we do not yet consider cases where the robot's beliefs can diverge, too, assuming that, while not being observed, humans only make deterministic moves. Hence, regardless of the co-presence, the robot always updates its belief with both the \textit{inferrable} and \textit{observable} effects.*
...
Note that we do not model explicit uncertainties in the human beliefs but somehow they are tackled anyway. Indeed, when such uncertainties should be present, particularly when the robot acts while the human is absent, we consider the human would believe that nothing has changed until they observe the changes. We are aware that a rational human will certainly not have such extreme reasoning but, without a complete blind trust in the robot, the human will be more or less uncertain about the robot non-observed action effects. Due to this uncertainty the human may have unpredictable reactions/initiatives at run-time that may be unnecessary or even detrimental to the joint task (e.g. trying to achieve a subtask the robot already did).
...
% Anthony-V2 WIP
The human and robot agents carry individual, distinct belief states w.r.t. the ground truth ($s$) (s.t. $\mathit{B}_{\varphi_r}^s$ is aligned with it), while the two can be either aligned or diverging. However, how explained above, the agents act as if they are certain about their knowledge.
To produce a legal joint-plan, the robot is fine with such belief divergences unless it is qualified as \textit{relevant} (Definition \ref{def:reldiv}). In such case, the relevant divergence needs to be tackle.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Apply sequentially actions of the plan from initial state and monitor the rel Inf attribute 
    \end{itemize}
...
% \begin{figure}[t!]
%     \centering
%     \includegraphics[width=0.50\linewidth]{figures/fig_delay.png}
%     \caption{
%     blabla delayed
%     }
%     \label{fig:scenarios}
...
*
A problem is deﬁned with a pair of initial
HR beliefs and a starting agent (H or R). In each domain, 5 attributes (each can get 2 values, while others get 1)
deﬁne the ground truth, R-belief. Among 5, 3 attr. can diverge in H-belief, generating 256 pairs of initial beliefs. Withthe starting agents, we have 512 problems for each domain. In each domain, the actions and tasks remain the
same.
*
