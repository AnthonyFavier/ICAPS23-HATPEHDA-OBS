...
For seamless human-robot collaboration, it is essential to integrate a task planning framework~\cite{lallement2018hatp} or a scheduling framework~\cite{ferreira2021scheduling} in an \text{online} joint behavior execution scheme. 
...
Suppose there exists a state-variable function,  $f_{svs}:(?g_1 (gr_1),..., ?g_k (gr_k),\mathcal{S}) \rightarrow ?g_{k+1} (gr_{k+1})$, such that for the current world state $s \in \mathcal{S}$, 
...
Due to all these limitations, this solution approach is only discussed qualitatively in this section and will not appear in the formal proofs and evaluations parts. 
