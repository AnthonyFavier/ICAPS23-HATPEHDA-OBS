...
Considering Theory of Mind (ToM) when executing a human-robot shared plan enables a robot to adapt its behavior online. Recent work proposes online replanning while tracking the progress of the task execution from both its own and humans' perspectives. 
If it finds a new plan, it communicates with the humans their part and other essential task details, and it starts to execute its own part. 
In this scenario, ToM is used to handle uncertainties caused mainly due to humans' \textit{unplanned} initiatives at run time, for example, their temporary absence or inattention, which prohibit humans to entail knowledge about the exact execution status. 
However, plan sharing can be costly, and committing every time to a new plan can be restrictive to humans or sometimes it may not be possible. Contemporary solvers lift the requirement of shared planning such that they only plan for the robot's behavior considering humans as uncontrollable agents and hence only anticipate their decisions and actions. Such estimation leads to an implicitly coordinated human-robot joint plan given a (shared) goal. 
...
In particular, we formalize a simple version of the situation assessment (SA) with perspective-taking, and use it in our new planning approach, i.e., propose where and how SA is inserted in the planning process. 
As a result, the solver produces more \textit{robust} robot plans while estimating the human mental state, encouraging the robot to communicate about relevant humans' false beliefs, but only minimally. 
We implement our new planning approach, prove its soundness and completeness, show its effectiveness qualitatively, and discuss experimental results on three novel benchmark domains. 
\end{abstract}
