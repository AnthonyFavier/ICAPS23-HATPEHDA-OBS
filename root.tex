...
DEL-based representation is rich and can model scenarios involving nested perspective-taking compared to our first-order representation used for maintaining agents' distinct beliefs. 
In~\cite{KR2021-12}, the notion of perspective shifts is extended to provide a foundation for producing implicitly coordinated human-robot plans such that they do not have to negotiate about and commit to a joint policy at plan time. 
In our understanding, this work produces HR policies that are not socially awkward, which is essentially the aim of such research, but they do not model humans as uncontrollable agents like ours.  
